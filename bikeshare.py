...
# -*- coding: utf-8 -*-
"""
This program is in order  to analyze the Bike user data from 3 different cities.
    ...
    while True:
        try:
            ...
            if code < min_value or code > max_value:
                print(
                '*> ERROR : Invalid code. Please enter a number in the given range (' + str(min_value) + '-' + str(
                    max_value) + ')')
            else:
                break
        except ValueError:
...
def get_filters():
    ...
    print(
    '*> Valid values are 1: Monday, 2: Tuesday, 3: Wednesday, 4: Thursday, 5: Friday, 6: Saturday, 7: Sunday, 8: All, 0:Exit')
...
def time_stats(df):
    ...
    try:
        ...
        print('**> Most common month is "{}" with {} travels'.format(MONTHS[busy_month_index - 1],
                                                                     busy_month[busy_month_index]))

        # display the most common day of week
        busy_day = df['day'].value_counts()
        busy_day_index = busy_day.keys()[0]
        print(
        '**> Most common day is "{}" with {} travels'.format(DAY_OF_WEEK[busy_day_index], busy_day[busy_day_index]))
...
def station_stats(df):
    ...
    try:
        ...
        print('**> Most commonly used start station is "{}" with {} travels'.format(station_index,
                                                                                    start_station[station_index]))

        # display most commonly used end station
        end_station = df['End Station'].value_counts()
        station_index = end_station.keys()[0]
        print(
        '**> Most commonly used end station is "{}" with {} travels'.format(station_index, end_station[station_index]))

        # display most frequent combination of start station and end station trip
        df['Start-End-Station'] = df['Start Station'] + '->' + df['End Station']
        popular_lines = df['Start-End-Station'].value_counts()
        line_index = popular_lines.keys()[0]
        print('**> Most frequent combination of start station is "{}" with {} trips'.format(line_index,
                                                                                            popular_lines[line_index]))
    except Exception as exp:
        print('**> ERROR : {}'.format(exp))
...
def user_stats(df):
    ...
    try:
        ...
        if nan_birth_years > 0:
            print('**> INFO : There are {} entry(ies) with Nan (Not a Number) value in Birth Date field'.format(
                nan_birth_years))
